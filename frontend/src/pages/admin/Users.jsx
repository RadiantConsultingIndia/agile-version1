...
export default function AdminUsers() {
  ...
  const grantHireCredits = async u => {
    const key = `${u.user_id}-hire`
    const amount = Number(creditInputs[key] ?? 20)
    if (!Number.isInteger(amount) || amount < 0) {
      toast('Enter a whole number of credits (0 or more)')
      return
    ...
    try {
      await api.post(`/api/admin/users/${u.user_id}/hire-credits?credits=${amount}`)
      setUsers(list => list.map(x => x.user_id === u.user_id ? { ...x, hire_credits: amount } : x))
      toast(`Set to ${amount} candidate invite credit${amount === 1 ? '' : 's'}`)
    } catch (err) {
      toast(err.response?.data?.detail || 'Failed to update Hire credits')
    }
  }
    ...
    <AdminLayout>
      ...
      <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #f1f5f9', boxShadow: '0 1px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        ...
        <table style={{ width: '100%', minWidth: 920, borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={9} style={{ padding: '60px 20px', textAlign: 'center', color: '#94a3b8', fontSize: 14 }}>No users found</td></tr>
            ) : filtered.map((u, i) => {
                ...
                <tr key={u.user_id} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f8fafc' }}>
                  ...
                  <td style={{ padding: '14px 20px' }}>
                    {u.role === 'employer' ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 50, whiteSpace: 'nowrap',
                          background: u.hire_credits > 0 ? '#f0fdf4' : '#f8fafc', color: u.hire_credits > 0 ? '#15803d' : '#94a3b8' }}>
                          {u.hire_credits ?? 0} left
                        </span>
                        <input type="number" min="0" step="1"
                          value={creditInputs[`${u.user_id}-hire`] ?? 20}
                          onChange={e => setCreditInputs(prev => ({ ...prev, [`${u.user_id}-hire`]: e.target.value }))}
                          style={{ width: 52, fontSize: 12, padding: '4px 6px', borderRadius: 6, border: '1.5px solid #e2e8f0', outline: 'none' }} />
                        <button onClick={() => grantHireCredits(u)}
                          style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6, border: 'none', cursor: 'pointer', background: '#059669', color: '#fff' }}>
                          Set
                        </button>
                      </div>
